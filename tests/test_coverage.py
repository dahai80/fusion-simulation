...
from fusion_simulation.sim.env import EngineType, EnvConfig, SimulationEnv
...
class TestCLICoverage:
    ...
    def test_scene_list(self):
        ...
        with patch.object(sys, "argv", ["fusion", "scene", "list"]):
            with patch(
                "fusion_simulation.sim.env.SimulationEnv.list_scenes",
                return_value=[{"name": "pick", "description": "Object picking task"}],
            ):
                main()
    ...
    def test_dataset_list_with_data(self):
        ...
        with patch.object(sys, "argv", ["fusion", "dataset", "list"]):
            with patch(
                "fusion_simulation.dataset.manager.DatasetManager.list",
                return_value=[{"name": "ds1", "engine": "lerobot", "sample_count": 100}],
            ):
                main()
    ...
    def test_dataset_import(self):
        ...
        with patch.object(
            sys, "argv", ["fusion", "dataset", "import", "--name=test", "--path=/tmp", "--engine=lerobot"]
        ):
            with patch(
                "fusion_simulation.dataset.manager.DatasetManager.import_dataset", return_value={"status": "imported"}
            ):
                main()
    ...
    def test_train(self):
        ...
        with patch.object(sys, "argv", ["fusion", "train", "--dataset=test", "--model-name=m1", "--epochs=2"]):
            with patch(
                "fusion_simulation.train.trainer.BCTrainer.train",
    ...
    def test_train_no_dataset(self):
        ...
        with patch.object(sys, "argv", ["fusion", "train", "--dataset=unknown", "--model-name=m1"]):
            with patch("fusion_simulation.dataset.manager.DatasetManager.get", return_value=None):
                with patch(
                    "fusion_simulation.dataset.manager.DatasetManager.collect_samples",
