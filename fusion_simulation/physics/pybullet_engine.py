...
class PyBulletEngine(PhysicsEngine):
    ...
    def init(self, config: PhysicsConfig | None = None, headless: bool = True) -> None:
        ...
        try:
            ...
            logger.info("PyBulletEngine initialized: client=%d, headless=%s, dt=%.4f",
                        self._client, headless, self._config.time_step)
        except ImportError:
    ...
    def load_urdf(self, urdf_path: str, position: list[float] | None = None,
                  ...
                  fixed_base: bool = False, use_fixed_base: bool = False) -> int:
        ...
        try:
            ...
            body_id = p.loadURDF(
                urdf_path, basePosition=pos, baseOrientation=orn,
    ...
    def set_body_position(self, body_id: int, position: list[float],
        ...
        try:
            ...
            p.resetBasePositionAndOrientation(
                body_id, position, orn, physicsClientId=self._client,
            )
        except Exception as e:
            logger.error("Failed to set position for body %d: %s", body_id, e)

    def apply_force(self, body_id: int, force: list[float],
        ...
        try:
            ...
            p.applyExternalForce(
                body_id, -1, force, pos, p.WORLD_FRAME,
                physicsClientId=self._client,
            )
        except Exception as e:
            ...
            for ji, val in zip(joint_indices, values):
                if mode == "position":
                    p.setJointMotorControl2(
                        body_id, ji, p.POSITION_CONTROL, targetPosition=val,
                        physicsClientId=self._client,
                    )
                elif mode == "velocity":
                    p.setJointMotorControl2(
                        body_id, ji, p.VELOCITY_CONTROL, targetVelocity=val,
                        physicsClientId=self._client,
                    )
                elif mode == "effort" or mode == "torque":
                    p.setJointMotorControl2(
                        body_id, ji, p.TORQUE_CONTROL, force=val,
                        physicsClientId=self._client,
                    )
        except Exception as e:
    ...
    def ray_test(self, origin: list[float], direction: list[float],
                 max_dist: float = 100.0) -> dict[str, Any] | None:
        self._ensure_initialized()
        try:
            import pybullet as p
    ...
    def get_camera_image(self, width: int = 640, height: int = 480,
                         ...
                         distance: float = 1.5, yaw: float = 45.0,
                         pitch: float = -30.0, fov: float = 60.0,
                         near: float = 0.1, far: float = 100.0) -> dict[str, Any]:
        self._ensure_initialized()
        try:
            import numpy as np
            import pybullet as p
